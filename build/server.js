...
`;
          ...
          tracer.log(`Sending response '${method} - (${message.id})'. Processing request took ${Date.now() - startTime}ms`, data2);
        } else {
      ...
      listen(connection2) {
        ...
        disposables.push(connection2.notebooks.synchronization.onDidChangeNotebookDocument((params) => {
          ...
          for (const close2 of closed) {
            removed.push(this.getNotebookCell(close2));
          }
          const textContent = [];
          for (const change2 of text) {
            textContent.push(this.getNotebookCell(change2));
          }
          if (added.length > 0 || removed.length > 0 || data2.length > 0 || textContent.length > 0) {
            changeEvent.cells = { added, removed, changed: { data: data2, textContent } };
          }
...
var addon = require(`../build/${os.platform()}-${os.arch()}.node`);
var update = addon.update;
var close = addon.close;
var validate = addon.validate;
var analyze = addon.analyze;
var parse = addon.parse;
var format = addon.format;
var help = addon.help;
...
// src/server.ts
var connection = ls.createConnection(ls.ProposedFeatures.all);
var documents = new ls.TextDocuments(TextDocument2);
...
documents.onDidChangeContent((event) => {
  ...
  const file = URI2.parse(uri).fsPath;
  if (!files.has(file))
    files.set(file, /* @__PURE__ */ new Set());
...
function getFunctionCompletion(name) {
  const detail = functionDetail(name);
  const help2 = help("function", name);
  return {
    label: name,
...
function getVariableCompletion(name) {
  const detail = variableDetail(name);
  const help2 = help("variable", name);
  return {
    label: name,
...
function getDirectoryCompletion(name) {
  return { label: name, kind: ls.CompletionItemKind.Folder };
}
function getFileCompletion(name) {
  return { label: name, kind: ls.CompletionItemKind.File };
}
function getLabelCompletion(name) {
  return { label: name, kind: ls.CompletionItemKind.Constant };
}
function getCompletions(file, line, column) {
  var _a;
  const result = [];
  const context = analyze(file, line, column);
  switch ((_a = context.token) == null ? void 0 : _a.type) {
    case "literal": {
      const detail = variableDetail(context.variable);
          ...
          try {
            if (colon) {
              if (detail.isLabel) {
                ...
                const scope = parse(filepath, content);
                scope.declares.forEach((declare) => {
                  ...
                  if (functionDetail(target).isTarget && label) {
    ...
    default: {
      result.push(...builtinFunctions().map(getFunctionCompletion));
      result.push(...builtinVariables().map(getVariableCompletion));
      if (context.function) {
        const func = context.function.name;
        const arg0 = (context.function.arguments[0] || "").replace(/^"|"$/g, "");
        const target = func == "target" ? arg0 : func;
        if (func == "template") {
          result.push(...targetFunctions().map(getFunctionCompletion));
        } else {
          result.push(...targetVariables(target).map(getVariableCompletion));
        }
      } else {
        result.push(...targetFunctions().map(getFunctionCompletion));
      }
...
function getHover(file, line, column) {
  var _a;
  const context = analyze(file, line, column);
  switch ((_a = context.token) == null ? void 0 : _a.type) {
    case "identifier": {
      const help2 = help("all", context.token.value);
      if (help2.basic) {
        return {
          contents: {
            kind: "markdown",
            value: [help2.full, help2.link].join("\n")
          },
...
function getDefinition(file, line, column) {
  var _a;
  const context = analyze(file, line, column);
  switch ((_a = context.token) == null ? void 0 : _a.type) {
    case "literal": {
      if (context.token.value.startsWith('"')) {
        ...
        const absolute = path.join(base, relative);
        const linkWithRange = (filepath, range) => {
          if (!range) {
            range = {
        ...
        try {
          ...
          if (entry.isFile()) {
            ...
            const content = fs.readFileSync(filepath, { encoding: "utf8" });
            const scope = parse(filepath, content);
            const declare = scope.declares.find((declare2) => {
              const func = declare2.function;
              const arg0 = (declare2.arguments[0] || "").replace(/^"|"$/g, "");
              const arg1 = (declare2.arguments[1] || "").replace(/^"|"$/g, "");
              const label = func == "target" ? arg1 : arg0;
              return label == target;
            });
...
function getFormatted(file, lines) {
  const code = format(file);
  if (code) {
    return {
      newText: code,
      range: {
...
function getDocumentSymbol(file) {
  const mapToDocumentSymbol = (symbol) => {
    const result = {
      name: symbol.name,
      kind: symbol.kind,
